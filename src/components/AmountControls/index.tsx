...
export function AmountControl({
  ...
  handleIncreaseAmount}: AmountControlProps) {
  return (
    <div>
    <div>
      <ButtonQuant onClick={handleDecreaseAmount} disabled={amount <= 1}>-</ButtonQuant>
      <input type="number" readOnly value={amount} />
      <ButtonQuant onClick={handleIncreaseAmount}>+</ButtonQuant>
    </div>
    <CartButton title="Adicionar ao carrinho">
      <ShoppingCart onClick={handleAddToCart} size={"1.125rem"} weight="fill" />
    </CartButton>
  </div>
  )
}